...
const ticketsReducer = (state: any = initialState, action: Action) => {
    ...
    switch(action.type){
        ...
        case ticketsActionType.EDIT:
            ...
            editedTicket.assignee_username= action.payload.assignee === undefined ? '' : action.payload.assignee.username
            editedTicket.assignee_user_discriminator= action.payload.assignee === undefined ? null : action.payload.assignee.discriminator
        ...
        case ticketsActionType.UNASSIGN_REMOVED_TEAMMATES_TICKETS:
            ...
            newTicketList.forEach((ticket: any, index)=>{
                if(ticket.assignee_username === action.payload.username && ticket.assignee_user_discriminator === action.payload.discriminator){
