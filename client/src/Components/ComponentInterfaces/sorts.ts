...
export const sorts: SortTypes = {project: 'project', status: 'status', priority: 'priority', title: 'title'}
...
function priorityCompare(i: ticket, j: ticket){
    return (i.priority < j.priority) ? -1 :
    (j.priority < i.priority) ? 1 : 
    0
}
function statusCompare(i: ticket, j: ticket){
    return (i.resolution_status < j.resolution_status) ? -1 :
    (j.resolution_status < i.resolution_status) ? 1 : 
    0
}
