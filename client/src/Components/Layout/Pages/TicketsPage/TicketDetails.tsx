...
export default function TicketDetails() {
    ...
    if(sessionState && isFocusedTicketUndefined === false){
        if(sessionState.currentTeam.team_role === 1){
            ...
            let projectIDX = sessionState.currentTeam.projects.findIndex((project: any) => {
                if(project.project_id === focusedTicketState.project_id && (project.role_id === 1 || project.role_id === 2)){
                    return true
                }
            })
            if(projectIDX !== -1){
    ...
    <div id='openFormDetailsHeight' className='ticketDetailsContainer '>
        <h4 className='header fadeIn ' style={{cursor: 'default'}}>{focusedTicketState.title}</h4>    
        <div className='ticketDetailsRow' >
            <div className='detailsColumn'>
                <div className='ticketDetailItemContainer'>
                    <div className='ticketDetailItem'>
                        <span className='detailUsername fadeIn '>{`${focusedTicketState.author_username}`} </span>
                        <span className='discriminator fadeIn '>#{focusedTicketState.author_discriminator}</span>
                        <span className='fadeIn ' style={{marginLeft: 'auto', marginRight: `${conditionalMarginRight}`}}>{moment(focusedTicketState.date_created).format('MMMM Do h:mma')}</span>
                        <div className='ticketListItemButtonContainer fadeIn scaleYonHover' onClick={()=>handleSetIsModalOpen()}>
                            {isUserAllowedToEdit && 
                            <>
                ...
                <div className='ticketDetailItemContainer '>
                    <div className='ticketDetailItem'>
                        ...
                        <span className='detailLabel fadeIn '  style={{marginLeft: 'auto'}}>Assignee:</span>
                        <span className='detailUsername fadeIn '>{`${assigneeUsername}`} </span>
                        <span className='discriminator fadeIn '>{isAssigneeNull ? '' : '#'}{assigneeDiscriminator}</span>
                        
                    </div>
                </div>    
                
                <div className='ticketDetailItemContainer'>
                    <div className='ticketDetailItem' style={{justifyContent: 'space-between'}}>
                        <span className='detailLabel fadeIn '>Priority:</span>
                        <span className='fadeIn '>{priority}</span>
                        
                        <span className='detailLabel fadeIn ' style={{marginLeft: 'auto'}}>Status:</span>
                        <span className='fadeIn '>{statusTranslation.translateTicketStatusBack(focusedTicketState.resolution_status)}</span>
                    </div>
                </div>       
            </div>
            <div id='detailsDescription' className='detailsColumn descriptionBottom detailsDescription '>
                <p className='ticketDescriptionArea fadeIn ' style={{width: '100%'}}>{focusedTicketState.description}</p>
            </div>
        </div>
    </div>
    }
