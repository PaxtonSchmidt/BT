...
export default function TicketNoteListItem(props: Props){
    ...
    let date = parseInt(props.note.date_created.substring(5,10).replace('-', ''))
    let hours = parseInt(props.note.date_created.toLocaleString().substring(11, 13))
    let minutes = parseInt(props.note.date_created.toLocaleString().substring(14, 16))
    ...
    let dateMonth = dateArray.replace(dateDay, '').split('').reverse().join().replace(/,/g,'')
