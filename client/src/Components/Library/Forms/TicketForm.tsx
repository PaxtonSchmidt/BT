...
export default function TicketForm(props: Props) {
  ...
  const { updateFocusedTicket, editFocusedTicket } = bindActionCreators(
    FocusedTicketActionCreators,
    dispatch
  );
  ...
  const { addTicket, editTicket } = bindActionCreators(
    TicketsActionCreators,
    dispatch
  );
    ...
    >
      {({ values, handleChange, handleBlur, handleSubmit, resetForm }) => {
        return (
          <form
            id='ticketForm'
            className={`${fadeAnimationType} form ${isFormTransitioned}`}
