...
async function submitNewTicket(req: any, res: any) {
  ...
  if (isAssigneeOnProject === false) {
    return res
      .status(403)
      .send({ message: 'The user youre assigning is not on that project...' });
  }
...
async function addListOfMembersToProject(req: any, res: any) {
  ...
  if (usersNotOnTeamIdxArray.length > 0) {
    return res
      .status(400)
      .send({ message: 'One or more of these users are not on the team...' });
  }
...
async function removeMember(req: any, res: any) {
  ...
  if (
    ...
    try {
      ...
      return res
        .status(500)
        .send({ message: 'Server couldnt remove member from the project...' });
    }
  } else {
    return res
      .status(403)
