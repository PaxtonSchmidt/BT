...
async function getSessionState(req: any, res: any) {
  ...
  let currentTeamID = consumeCookie(
    req.headers.cookie,
    consumeCookieFlags.tokenTeamIdFlag
  );
  ...
  try {
    ...
    sessionInvites = userInvites.map((invite: any) =>
    ...
    if (currentTeam.team_role === 1) {
      let projectsUserIsIn = await project.getSessionProjectRoles(
        currentTeamID,
        currentUserID
      );
      let allProjects = await project.getAllProjectsOnTeam(
        currentTeamID,
        currentUserID
      );
      ...
      userProjects = await project.getSessionProjectRoles(
        currentTeamID,
        currentUserID
      );
    }
    ...
    if (currentTeam.team_role === 1) {
      ...
      projectMembers = await project.getProjectMembers(
        currentUserID,
        currentTeamID
      );
    }
    projectMembers.map((project: any) => Object.assign({}, project));
    projectMembers = projectMembers.map((project: any) =>
      Object.assign({}, project)
    );
    ...
    userProjects.forEach((project: any) => {
      let projectInstance = {
        ...
        project_members: projectMembers.filter(
          (projectMember: any) =>
    ...
    return res
      .status(500)
      .send({ message: 'Server couldnt fetch user information...' });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .send({ message: 'Server couldnt retrieve user details...' });
  }
}
