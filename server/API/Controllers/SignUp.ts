...
async function signUp(req: any, res: any) {
    ...
    try{
        ...
        isUsernameAndDiscComboTakenPacket = await signUpQueries.isUsernameAndDiscComboTaken(req.body.username, req.body.discriminator).catch()
        isUsernameAndDiscComboTaken = consumeRowDataPacket(isUsernameAndDiscComboTakenPacket)
    } catch(e){
        return res.status(500).send({message: 'Server couldnt check information...'})
    }
    if(isEmailTaken){
        return res.status(400).send({message: 'That email is already in use...'})
    } else if(isUsernameAndDiscComboTaken){
        return res.status(400).send({message: 'That username and discriminator combination is already taken...'})
    }else{
        try{
            let date = getCurrentDate()
            await userController.addUser(req.body.username, req.body.discriminator, req.body.password, req.body.email, date, req.body.bio, '0').catch()
