...
export default function composeTeammateInfo(info: InfoItem[], user_projects: UserProject[]){
    ...
    for(let i = 0; i < user_projects.length; i++){
        let teammateIDX: number = teammatesInfo.findIndex((teammate: TeammatesInformation) => {
            if(teammate.username === user_projects[i].username && teammate.discriminator === user_projects[i].discriminator){
                return true
            }
        })
        teammatesInfo[teammateIDX].projects.push({project_name: user_projects[i].name, role_id: user_projects[i].role_id})
    }
